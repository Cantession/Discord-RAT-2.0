...
Once the bot is created open ```Client-built.exe``` as admin in the build file and then execute ```build.exe```. paste the token in, and paste the guild ID of where you invited the bot and click build It will create a new channel and post a message on the server with a generated session number.\
